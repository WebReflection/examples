...
console.time("createEmscriptenModule");
const modulePromise = await createEmscriptenModule(emscriptenSettings);
console.timeEnd("createEmscriptenModule");
...
onmessage = async (event) => {
  if (event.data.type === "run") {
    ...
    console.time("callMain");
    const ret = Module.callMain(event.data.args);
    console.timeEnd("callMain");
    postMessage({
      type: "finished",
      returnCode: ret,
    });
